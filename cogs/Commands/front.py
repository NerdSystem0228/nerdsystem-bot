...
class Front(commands.Cog):
    ...
    async def front(self, interaction: Interaction):
        ...
        r= await self.bot.APIHttp.set_alter_front(member)
        if isinstance(r, dict):
            await interaction.response.send_message(embed=self.create_error_embed("Você já está no front"), ephemeral=True)
            return
        if r:
    ...
    def create_message_embed(self, colour):
        embed=Embed(title=f"Você entrou no front!", colour=colour(), timestamp=dt.datetime.now())
        embed.set_author(name=self.bot.user.display_name, icon_url=self.bot.user.avatar.url)
        return embed

async def setup(bot):
    await bot.add_cog(Front(bot), guilds=[discord.Object(id=bot.SYSTEM_SERVER)])
