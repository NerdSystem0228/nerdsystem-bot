...
class OnMemberJoin(commands.Cog):
    ...
    async def on_member_join(self, member): 
        ...
        await bot.get_channel(data.WELCOME_CHANNEL).send(member.mention, embed=embed)
    
    def create_welcome_embed(self, member):
        embed=Embed(title=f"Bem vindo ao {bot.get_guild(data.SYSTEM_SERVER).name}!", color=discord.Color.blurple(), timestamp=dt.datetime.now())
        embed.description=f"Espero que goste de sua estadia nesse hospí- quero dizer, lugar com pessoas acolhedoras que tem mente aberta o suficiente para aceitar a existência de uma estrutura neural onde possibilita mais de uma consciência num mesmo cérebro, parabéns de agora ser parte desse grupo! Por favor, leia as {bot.get_channel(data.RULE_CHANNEL).mention} e não seja um cuzão!"
